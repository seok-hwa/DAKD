...
class MapillaryDataset(BaseDataset):
    ''' The dataset class for Mapillary '''
    ...
    def __init__(self,
        ...
        super(MapillaryDataset, self).__init__(root, num_classes, crop_size, label_size, mean, domain)
        self.dataset_name = 'Mapillary'

        # Get a list of image and label paths
        self.split = 'training' if train else 'validation'
        img_paths = sorted(self.root.glob('{}/images/*.jpg'.format(self.split)))
        
        for img_path in img_paths:
            img_path, lbl_path, img_name = self.get_metadata(img_path)
            self.files.append((img_path, lbl_path, img_name, domain))
    ...
    def get_metadata(self, img_path):
        ...
        *dir_path, img_name = str(img_path).split('/')
        ...
        lbl_name = img_name.replace('.jpg', '_labelTrainIds{}.png'.format(self.num_classes))
